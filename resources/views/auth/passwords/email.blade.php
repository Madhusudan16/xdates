                ...
                <div class="col-md-offset-3 col-md-6">
                    <div class="forgot_form text-center">
                       <p>Enter your email address below and we'll send password reset instructions.</p>
                        <form class="form-inline" autocomplete="off" role="form" method="POST" action="{{ url('/password/email') }}">
                        	{!! csrf_field() !!}
                              <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }} ">
                                <label class="sr-only" for="exampleInputEmail3">Email address</label>
                                <span class="border-span"><input type="email" class="form-control email"  name="email" value="{{ old('email') }}"></span>
