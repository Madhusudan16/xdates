...
class PasswordController extends Controller
{
    ...
    */
    ...
    public function sendResetLinkEmail(Request $request)
    {
        ...
        $resultCount = User::where(['email'=>$email])->get()->first();
    ...
    public function showLinkRequestForm()
    {
        ...
        if (view()->exists('auth.passwords.email')) {
            return view('auth.passwords.email', ['validator'=>$validator]);
        }
		
		
        return view('auth.password', ['validator'=>$validator,'page_title'=>'Reset Password']);
    }
    ...
    public function showResetForm(Request $request, $token = null)
    {
        ...
        if (is_null($token)) {
            ...
            $result = DB::table('password_resets')->where(['email'=>$email,'token'=>$token])->count();
            if($result !=1) {
        ...
        if (view()->exists('auth.passwords.reset')) {
            return view('auth.passwords.reset')->with(compact('token', 'email','validator'));
        }
		
        return view('auth.reset')->with(compact('token', 'email','validator'));
    }
	...
	{
	    return redirect('/password/reset/send')->with('status', trans($response));
	}
}
